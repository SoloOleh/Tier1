...
# У вимогах для боту ДЗ№3 ці функції із ДЗ№2 не використовуюються для команд інтерфейсу користувача, тому я іх закоментував.
# Якщо вони всежтаки потрібні, тоді розкоментуйте їх будь ласка, вони готові до використання.         
...
def main():
    ...
    while True:
        ...
        if command == "close" or command == "exit":
            ...
            word = ' '.join(args)
            found_notes = notes_manager.search_notes(word)
            if found_notes:
                for note in found_notes:
                    print(f"ID: {note.note_id}, Title: {note.title}, Description: {note.description}, Tags: {note.tag}, Date: {note.date.strftime('%d.%m.%Y')}")
            else:
                print("No notes found with that word.")
        else:
